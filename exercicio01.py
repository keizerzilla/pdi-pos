...
for i in images:
	...
	cv2.imwrite("exercicio01/" + i.replace(".jpg", "_raw.jpg"), img)
...
for i in images:
	...
	cv2.imwrite("exercicio01/" + i.replace(".jpg", "_redim.jpg"), mod)

# binarizacao
for i in images:
	img = cv2.imread(i, cv2.IMREAD_GRAYSCALE)
	ret, mod = cv2.threshold(img, 127, 255, cv2.THRESH_BINARY)
	cv2.imwrite("exercicio01/" + i.replace(".jpg", "_binary.jpg"), mod)
...
for i in images:
	...
	mod = cv2.resize(mod, (512, 512), interpolation=cv2.INTER_LINEAR)
	cv2.imwrite("exercicio01/" + i.replace(".jpg", "_redim_binary.jpg"), mod)
